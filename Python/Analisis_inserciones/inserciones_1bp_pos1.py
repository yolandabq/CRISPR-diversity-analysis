...
Script to analyze what nucleotides are usually inserted after the base X (A,C,G,T) in position -1. 
...
#%% We save the nucleotide inserted depending on the nucleotide in the position -1. 
# Cualitative analysis (we don't consider their frequencies)

# We take into account the alleles 1:1I. However, we consider too the alleles -1:1I and 2:1I as they can be sometimes be realigned. 

nt_list = {} # We save the nucleotides depending on the base in position -1. 
nt_list["A"] = []
nt_list["C"] = []
nt_list["T"] = []
nt_list["G"] = []
...
for carpeta in ["results", "results_no_replic"]:
    ...
    for g_id in guides_id:
        ...
        # If there is -1:1I or 2:1I, we see if the alignment can be changed, and if it can, we name them 1:1I.
...
nt = "A" # A, C, G o T. Cambiar para obtener los otros
...
#%% Cuantitative analysis. Now, we save the frequencies, not only which nt is the most commonly inserted. 
...
#%% Let's see how many guides have their majority allele 1:1I according to their nt at pos -1
# We normalize with respect the number of guides with that nt in pos -1. 

# That is, of the number of guides that have A in minus 1, how many of them have 1:1I as the majority allele
...
for x, y in nt_list_mayor.items():
    labels.append(x)
    porc.append(len(y)/number_guides[x]) 
