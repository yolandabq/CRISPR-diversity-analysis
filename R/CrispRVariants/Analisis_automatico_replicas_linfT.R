...
# Analysis of the guides that have 3 or more replicates. They go to the folder "results" and "results_ins".
...
for (gene in unique(guide_data$genename)){
  ...
  guides_name <-paste(guides, collapse=',' ) # the "," separates the bam from different guides, and the ";" separate the bams within the same guide
...
#for (replic in c(1:5)){
  ...
  for (guide_id_name in list_guide_id_file){
    ...
    for (guide_file_name in gRNA_files_name){
      ...
      file_path <- str_c("C:/Users/yolib/Documents/TFM/Linf_T/Datos/results/", replic, "_", guide_id_name, ".csv") # para guardar el csv
      ...
      file_path <- str_c("C:/Users/yolib/Documents/TFM/Linf_T/Datos/results_ins/", replic, "_", guide_id_name, "_ins.csv") # para guardar el csv
      
      write.csv(insertions, file_path, row.names = FALSE)
      # ----------------------------------------------------------------------------------------------
    }
    
    ref_data <- rbind(ref_data, c(as.character(replic), as.character(reference), as.character(strand)))
  }
  
  colnames(ref_data) = c("Guide_ID", "Sequence", "Strand")
  file_path <- str_c("C:/Users/yolib/Documents/TFM/Linf_T/Datos/guides_sequence_2.csv") # para guardar el csv
